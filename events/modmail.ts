...
Note: If your message becomes more than 4096 characters long, the preview embed will be cut off to that length. However, any overflowing messages will still be sent to the mods.`),
  INTRO: new MessageEmbed()
    ...
    .setTitle("Modmail Instructions")
    .setDescription(
      "Welcome to modmail! type `!modmail` to start a new modmail."
    ),

  MESSAGE_COLOR: "#00ffff" as ColorResolvable,
  MESSAGE_TITLE: "Modmail Message",

  DEFAULT_MODMAIL_TEXT:
    "Start sending messages to be included in the modmail, and they will show up here replacing this message in the preview.",
  ...
  MODMAIL_FAILED: new MessageEmbed()
    ...
    .setDescription(
      "Something went wrong when sending the modmail. Please try again later."
    ),
  ...
  MODMAIL_CANCELED_NEW: new MessageEmbed()
    ...
    .setTitle("Modmail Canceled")
    .setDescription(
      "This modmail has been canceled because you started a new one."
    ),
...
function setCancellationTimeout(
  ...
  const timeoutHandle = setTimeout(() => {
    ...
    dmChannel
      .send({ embeds: [constants.MODMAIL_CANCELED_INACTIVITY.setTimestamp()] })
      .catch(console.error);
    previewMessage
...
function getPreviewContent(modmailContent: string) {
  return modmailContent.length > 4096
    ? modmailContent.slice(0, 4096 - 4) + " ..."
    : modmailContent;
}
...
async function newModmail(
  ...
  const author = await server.members
  ...
  // set timeout
  const timeoutHandle = setCancellationTimeout(
    activeMessages,
    message.author.id,
    message.channel as DMChannel, // not sure if this is the best way to do this
    previewMessage
  );
...
async function updateModmail(
  ...
  await previewMessage.edit({
    embeds: [newPreviewEmbed],
    components: [constants.BUTTON_ROW],
  });

  // renew timeout
  clearTimeout(timeoutHandle);
  const newTimeoutHandle = setCancellationTimeout(
    activeMessages,
    message.author.id,
...
export const previewContent = getPreviewContent;
export const cancellationTimeout = setCancellationTimeout;
